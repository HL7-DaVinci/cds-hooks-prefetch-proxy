...
function processPrefetchQueryTemplate(prefetchQueryTemplate, ptd) {
  const tokens = getHandlebarTokens(prefetchQueryTemplate);
  for (let token of tokens) {
    const resolvedToken = resolvePrefetchToken(token, ptd);
    if (!resolvedToken) {
      console.log("Could not resolve token in query " + prefetchQueryTemplate);
      return false;
    }
...
module.exports = async function hydratePrefetch(req_body, service_info) {
  ...
  for (let prefetchKey in service_info.prefetch) {
    if (req_body.prefetch[prefetchKey] != null) continue; //this prefetch key already hydrated
    console.log("need to hydrate " + prefetchKey);
    let prefetchQueryTemplate = service_info.prefetch[prefetchKey];
    let prefetchQuery = processPrefetchQueryTemplate(prefetchQueryTemplate, ptd);
  ...
  try {
    ...
    for (let i = 0; i < responses.length; i++) {
      ...
      console.log("Successfully hydrated prefetch key " + prefetchKey + " with data: ");
      console.log(JSON.stringify(response.data, null, 2));
      req_body.prefetch[prefetchKey] = response.data;
    }
  } catch (error) {
    console.log("Error executing fhir query:" + error.toString());
  }
};
